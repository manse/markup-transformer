...
module.exports = function(code, options) {
	...
	function generateCode(list) {
		...
		function generateBlock(width, startAt, eol, tailMustPadding) {
			var position = [];
			var length = 0;
		...
		function generateLine(map, startAt) {
			var results = [];
			var retries = 5;
			for (var retry = 0; retry <= retries; retry++) {
				var lastTry = retry == retries;
				var start = startAt;
				...
				for (var i = 0, ii = map.length; i < ii; i++) {
					if (!map[i].fill) {
						...
						var b = generateBlock(map[i].width, start, i + 1 == ii, lastTry);
						if (!b) {
							killed = true;
							break;
						}
		...
		var kill = false;
		for (var i = 0; !kill; i++) {
			var line = generateLine(parseMap(options.shape(i)), startAt);
